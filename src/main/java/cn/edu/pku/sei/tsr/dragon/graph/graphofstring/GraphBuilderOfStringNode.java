...
public class GraphBuilderOfStringNode {
	public static final Logger	logger		= Logger.getLogger(GraphBuilderOfStringNode.class);
	public static final int		EDGETYPE	= 0;

	public static ListGraph<String, Integer> buildGraph(VerbalPhraseStructureInfo vps) {
	 ...
	 * @return
	 */
	public static ListGraph<String, Integer> convertToParsemisGraph(GraphInfo graphInfo) {
		if (graphInfo == null || graphInfo.getNodeList().size() <= 0)
			return null;

		ListGraph<String, Integer> graph = new ListGraph<>(graphInfo.getName());
		...
		for (int i = 0; i < graphInfo.getNodeList().size(); i++) {
			...
			if (node.getUuid() == null)
				node.setUuid();
			Node<String, Integer> addedNode = graph.addNode(node.toString());
			nodeMap.put(node.getUuid(), addedNode);
		}

		for (EdgeInfo edge : graphInfo.getEdgeList()) {
			Node<String, Integer> nodeA = nodeMap.get(edge.getNodeA().getUuid());
			Node<String, Integer> nodeB = nodeMap.get(edge.getNodeB().getUuid());
	...
	public static void main(String[] args) {
		...
		for (PhraseInfo phrase : sentence.getPhrases()) {
			...
			System.err.println(graphInfo);
			Graph<String, Integer> g = convertToParsemisGraph(graphInfo);
			System.err.println(GraphUtils.toString(g));
			Iterator<Node<String, Integer>> ite = g.nodeIterator();
			while (ite.hasNext()) {
				System.err.println("------------");
				Node<String, Integer> n = ite.next();
				System.err.println(n.getLabel());
				System.err.println(GraphUtils.parseStringToNodeInfo(n.getLabel()));
			}
		...
		System.out.println("=================");
		System.err.println(graphInfo);
		System.err.println(GraphUtils.toString(convertToParsemisGraph(graphInfo)));
