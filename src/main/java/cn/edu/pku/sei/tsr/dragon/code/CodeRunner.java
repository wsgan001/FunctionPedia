...
import cn.edu.pku.sei.tsr.dragon.statistics.MainParser;
import cn.edu.pku.sei.tsr.dragon.utils.Config;
import cn.edu.pku.sei.tsr.dragon.utils.ObjectIO;
import cn.edu.pku.sei.tsr.dragon.visitor.ContentVisitor;

public class CodeRunner{
	public static final Logger logger = Logger.getLogger(CodeRunner.class);
	...
	public static void parseProject(String path, String name) {
		...
		logger.info("code data write over");
...
//		javaPackage.accept(visitor);
	...
	public static void main(String[] args) {
		...
		projects.add(new ImmutablePair<String, String>("poi-3.13\\src\\java", APILibrary.APACHE_POI));
...
//		projects.add(new ImmutablePair<String, String>("struts-2.3.24\\src\\core\\src\\main\\java", APILibrary.STRUTS2));
		...
		projects.forEach(project -> {
			logger.info(project.getRight() + " start");
			long t1 = System.currentTimeMillis();
			parseProject(Config.getDataRootDir()+ File.separator + ObjectIO.SUBJECTS + File.separator + project.getLeft(), project.getRight());
			long t2 = System.currentTimeMillis();
			logger.info(project.getRight() + " end in "+(t2-t1)+"ms");
		});
	}
